...
| 1.0.23 | user kembali "masih blue screen": dua akar baru — (1) workflow salah pakai preset "Android AIO" (APK berisi bundle konten → "tak ada unduhan sama sekali" dan menyalahi aturan launcher-murni); (2) `_static_walk` sinkron total bisa membekukan layar pada ponsel lemah (watchdog takdapat memberi jejak) | (1) preset AIO DIHAPUS dari export_presets + workflow export "Android Launcher" (APK kecil; konten SELALU dari server pack; APK tak disentuh lagi); (2) `_static_walk` jadi async, yield tiap 12 mesh + progress "mesh ke-N"; (3) watchdog 40 dtk: bila boot macet → panel merah menampilkan tahap terakhir + ekor log + info perangkat; (4) log boot permanen ke user://boot_log.txt (device, renderer, resolusi, setiap tahap) |
